...
Perf.Distance = [Perf.Distance Distance_Final/1000];
Perf.kWh_batt = [Perf.kWh_batt kWh_Final];
Perf.Conso_elec = [100*Perf.kWh_batt(1)/Perf.Distance(1) 100*sum(Perf.kWh_batt)/sum(Perf.Distance)];
...
if Distance_Final(last_index)<=0.98*max(CYCL.position)
    Track_test.main = 'Track no finished';
else
    Track_test.main = 'Track completed';
end

Track_test.Time = Time_Final;
Track_test.Distance = Distance_Final/1000;
